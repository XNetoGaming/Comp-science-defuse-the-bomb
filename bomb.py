...
def setup_phases():
    ...
    keypad = Keypad(component_keypad)
    # setup the jumper wires thread
    wires = Wires(component_wires)
    # setup the pushbutton thread
    button = Button(component_button_state, component_button_RGB, button_target, button_color, timer)
    # bind the pushbutton to the LCD GUI so that its LED can be turned off when we quit
    gui.setButton(button)
    # setup the toggle switches thread
    toggles = Toggles(component_toggles)
    ...
    # Trigger binary code flashing after setup phases are started
    button.flash_binary_codes()
...
def explode():
    audio.play_explosion_sound()  # Play explosion sound
    gui.display_explosion()  # Show explosion in GUI
    turn_off_all_phases()

def defuse():
    audio.play_defusal_sound()  # Play defusal success sound
    gui.display_defusal_success()  # Show success message in GUI
    turn_off_all_phases()
# turns off the bomb
def turn_off():
    # stop all threads
